...
const filteredSearchPlugins = computed(() => {
  ...
  return props.categorizedPlugins.map((category) => {
    // for each category, filter to only the groups with plugins matching the query
    const filtered = category.pluginGroups.map((group) => ({
      ...group,
...
<template>
  ...
  <div class="drawer-side h-full shadow-xl">
    ...
    <div class="bg-base-100 border-base-300 flex min-h-full w-full flex-col border-r sm:w-lg">
      ...
      <header class="border-base-300 bg-base-100 sticky top-0 z-10 flex items-center justify-between border-b p-6">
        <div>
      ...
      <div class="flex-1 overflow-y-auto p-4">
        ...
        <div v-else>
            ...
            <div class="card-body">
              ...
              <div class="flex flex-col gap-2 mt-2">
                <label class="label cursor-pointer justify-start gap-4">
                  <input type="radio" class="radio radio-primary" :checked="!quietMode" @change="quietMode = false" />
                  <span class="label-text font-medium">Verbose</span>
                </label>
                <label class="label cursor-pointer justify-start gap-4">
                  <input type="radio" class="radio radio-primary" :checked="quietMode" @change="quietMode = true" />
          ...
          <div v-for="category in filteredSearchPlugins" :key="category.name"
